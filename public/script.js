document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchIssuedBooks = async () => {
        ...
        books.forEach((book) => {
            ...
            card.innerHTML = `
                <h3>${book.name}</h3>
                <p><strong>Issued At:</strong> ${new Date(book.issuedAt).toLocaleString()}</p>
                <p><strong>Return By:</strong> ${new Date(new Date(book.issuedAt).getTime() + 60000).toLocaleString()}</p>
                <p><strong>Fine:</strong> ${fine > 0 ? fine + ' INR' : 'No fine'}</p>
    ...
    const fetchReturnedBooks = async () => {
        ...
        books.forEach((book) => {
            ...
            card.innerHTML = `
                <h3>${book.name}</h3>
                <p><strong>Issued At:</strong> ${new Date(book.issuedAt).toLocaleString()}</p>
                <p><strong>Returned At:</strong> ${new Date(book.returnedAt).toLocaleString()}</p>
